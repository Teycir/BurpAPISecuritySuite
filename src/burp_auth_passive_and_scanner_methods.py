...
def _stop_vulners(self, event):
    """Request cancellation for Vulners enrichment run."""
    self._set_tool_cancel("vulners")
    self.vulners_area.append("[!] Vulners stop requested by user\n")
    self.log_to_ui("[!] Vulners stop requested by user")
...
def _extract_vulners_signature_from_value(self, raw_value):
    """Parse product/version software signature from one header/body signal value."""
    value = self._ascii_safe(raw_value or "").strip()
    if not value:
        return None
    ...
    match = re.search(
        r"([A-Za-z][A-Za-z0-9+._-]{1,63})[/\s]v?([0-9][0-9A-Za-z._-]{0,31})",
        clean,
    )
    if match:
        product = self._ascii_safe(match.group(1) or "").strip()
        version = self._ascii_safe(match.group(2) or "").strip()
    else:
        token = re.split(r"[;(),\s]+", clean, 1)[0]
        product = self._ascii_safe(token or "").strip()
        version = ""

    product = re.sub(r"[^A-Za-z0-9+._-]", "", product)
    version = re.sub(r"[^A-Za-z0-9._-]", "", version)
    if not product:
        return None
    signature = product if not version else "{} {}".format(product, version)
    return {
        "signature": signature,
        "product": product,
        "version": version,
...
def _collect_vulners_fingerprints(self, max_signatures):
    """Collect software/version fingerprints from filtered captured traffic."""
    max_items = self._parse_positive_int(str(max_signatures), 20, 1, 80)
    ...
    try:
        ...
        self._callbacks.printError(
            "Vulners filtered-view snapshot error: {}".format(str(e))
        )
        filtered_source = {}
    ...
    for endpoint_key, raw_entries in filtered_source.items():
        ...
        for item in entries:
            ...
            host = self._ascii_safe(entry.get("host"), lower=True).strip()
            path = self._ascii_safe(entry.get("path") or "/").strip() or "/"
            if not host:
                continue
            url = "{}://{}{}".format(protocol, host, path)
            normalized_headers = self._normalize_header_dict(
                entry.get("response_headers") or {}
            )

            for header_name, header_value in normalized_headers.items():
                name_key = self._ascii_safe(header_name, lower=True).strip()
                if name_key not in header_candidates:
                    continue
                parsed = self._extract_vulners_signature_from_value(header_value)
                if not parsed:
                    continue
                signal_count += 1
                dedup_key = "{}|{}".format(
                    self._ascii_safe(parsed.get("product"), lower=True),
                    self._ascii_safe(parsed.get("version"), lower=True),
                )
                item_bucket = signatures.get(dedup_key)
                if item_bucket is None:
                    item_bucket = {
                        "signature": self._ascii_safe(parsed.get("signature") or "").strip(),
                        "product": self._ascii_safe(parsed.get("product") or "").strip(),
                        "version": self._ascii_safe(parsed.get("version") or "").strip(),
                        "sample_urls": set(),
                        "sources": set(),
                        "signals": 0,
                    }
                    signatures[dedup_key] = item_bucket
                item_bucket["sample_urls"].add(url)
                item_bucket["sources"].add(name_key)
                item_bucket["signals"] = int(item_bucket.get("signals", 0) or 0) + 1
            ...
            if generator_match:
                parsed = self._extract_vulners_signature_from_value(
                ...
                if parsed:
                    ...
                    if item_bucket is None:
                        ...
                        signatures[dedup_key] = item_bucket
                    item_bucket["sample_urls"].add(url)
                    item_bucket["sources"].add("meta-generator")
                    item_bucket["signals"] = int(item_bucket.get("signals", 0) or 0) + 1

    normalized = []
    for item in signatures.values():
        urls = sorted(item.get("sample_urls", set()))
        sources = sorted(item.get("sources", set()))
        normalized.append(
            {
                "signature": self._ascii_safe(item.get("signature") or "").strip(),
                "product": self._ascii_safe(item.get("product") or "").strip(),
                "version": self._ascii_safe(item.get("version") or "").strip(),
                "sample_urls": urls[:5],
                "sources": sources,
    ...
    normalized.sort(
        key=lambda row: (
            ...
            self._ascii_safe(row.get("signature") or "", lower=True),
        ),
        reverse=True,
    )
...
def _vulners_fetch_target_material(self, target_url, timeout_seconds=10):
    """Fetch one target URL and return response headers/body for fingerprint extraction."""
    clean_url = self._clean_url(target_url)
    if not clean_url:
    ...
    try:
        ...
        request.add_header("User-Agent", "BurpAPISecuritySuite/1.0")
        request.add_header("Accept", "text/html,application/json,*/*")
        response = urllib2.urlopen(request, timeout=timeout_seconds)
        body_text = self._ascii_safe(response.read() or "")
        headers = {}
        try:
            for key, value in (response.info().items() or []):
                headers[self._ascii_safe(key or "")] = self._ascii_safe(value or "")
        except Exception as header_err:
            self._callbacks.printError(
                "Vulners custom-target header parse error for {}: {}".format(
                    clean_url, str(header_err)
                )
            )
        ...
        return {}, "", "HTTP request failed: {}".format(self._ascii_safe(e))

    try:
        import urllib.request as urllib_request

        request = urllib_request.Request(clean_url)
        request.add_header("User-Agent", "BurpAPISecuritySuite/1.0")
        request.add_header("Accept", "text/html,application/json,*/*")
        ...
        headers = {}
        try:
            for key, value in (response.headers.items() or []):
                headers[self._ascii_safe(key or "")] = self._ascii_safe(value or "")
        except Exception as header_err:
            self._callbacks.printError(
                "Vulners custom-target header parse error for {}: {}".format(
...
def _collect_vulners_fingerprints_from_custom_targets(
    self, targets, max_signatures, cancel_check=None
):
    """Collect fingerprints by actively fetching Vulners custom target base URLs."""
    max_items = self._parse_positive_int(str(max_signatures), 20, 1, 80)
    ...
    for target_url in target_list:
        if callable(cancel_check):
            try:
                ...
                self._callbacks.printError(
                    "Vulners cancel-check error: {}".format(str(cancel_err))
                )
        headers_map, response_body, fetch_error = self._vulners_fetch_target_material(
            target_url, timeout_seconds=10
        )
        if fetch_error:
            fetch_errors.append(
                "{} -> {}".format(self._ascii_safe(target_url), self._ascii_safe(fetch_error))
            )
            continue
        fetched_count += 1
        normalized_headers = self._normalize_header_dict(headers_map or {})
        ...
        for header_name, header_value in normalized_headers.items():
            ...
            if item_bucket is None:
                ...
                signatures[dedup_key] = item_bucket
            item_bucket["sample_urls"].add(self._ascii_safe(target_url))
            item_bucket["sources"].add("custom-{}".format(name_key))
            item_bucket["signals"] = int(item_bucket.get("signals", 0) or 0) + 1

        generator_match = re.search(
            r'(?is)<meta[^>]+name=["\']generator["\'][^>]+content=["\']([^"\']+)["\']',
            self._ascii_safe(response_body or ""),
        )
        if generator_match:
            parsed = self._extract_vulners_signature_from_value(generator_match.group(1))
            if parsed:
                signal_count += 1
                dedup_key = "{}|{}".format(
                    self._ascii_safe(parsed.get("product"), lower=True),
                ...
                if item_bucket is None:
                    ...
                    signatures[dedup_key] = item_bucket
                item_bucket["sample_urls"].add(self._ascii_safe(target_url))
                item_bucket["sources"].add("custom-meta-generator")
                item_bucket["signals"] = int(item_bucket.get("signals", 0) or 0) + 1
    ...
    meta = {
        "source_count": len(target_list),
        "fetched_count": fetched_count,
        "signal_count": signal_count,
        "fingerprint_count": len(normalized),
        "fetch_error_count": len(fetch_errors),
        "fetch_error_details": fetch_errors[:10],
    }
    return normalized, meta

def _vulners_http_get_json(self, base_url, params, timeout_seconds=25):
    """HTTP GET JSON helper with Jython/Python fallback and strict error surface."""
    ...
    try:
        ...
        full_url = "{}?{}".format(base_url, query)
        request = urllib2.Request(full_url)
        request.add_header("Accept", "application/json")
        request.add_header("User-Agent", "BurpAPISecuritySuite/1.0")
        response = urllib2.urlopen(request, timeout=timeout_seconds)
        payload_text = self._ascii_safe(response.read())
    except ImportError:
        try:
            ...
            full_url = "{}?{}".format(base_url, query)
            request = urllib_request.Request(full_url)
            request.add_header("Accept", "application/json")
            request.add_header("User-Agent", "BurpAPISecuritySuite/1.0")
            response = urllib_request.urlopen(request, timeout=timeout_seconds)
            payload_text = self._ascii_safe(response.read())
            ...
            raise RuntimeError(
                "HTTP request failed: {}".format(self._ascii_safe(fallback_err))
            )
    except Exception as e:
        raise RuntimeError("HTTP request failed: {}".format(self._ascii_safe(e)))

    text = self._ascii_safe(payload_text or "").strip()
    if not text:
        raise RuntimeError("Empty response from Vulners API")
    if text.startswith("<"):
        snippet = self._ascii_safe(text[:120]).replace("\n", " ")
        if "just a moment" in self._ascii_safe(text, lower=True):
            raise RuntimeError(
                "Vulners request blocked by anti-bot challenge (Cloudflare): {}".format(
                    snippet
                )
            )
        raise RuntimeError("Non-JSON response from Vulners API: {}".format(snippet))
    try:
        return json.loads(text)
    except (TypeError, ValueError) as json_err:
        snippet = self._ascii_safe(text[:160]).replace("\n", " ")
        raise RuntimeError(
            "Invalid JSON from Vulners API: {} | payload={}".format(
                self._ascii_safe(json_err), snippet
            )
        )

def _normalize_vulners_doc(self, raw_doc):
    """Normalize one Vulners search document into stable finding fields."""
    doc = raw_doc if isinstance(raw_doc, dict) else {}
    finding_id = self._ascii_safe(
        doc.get("id") or doc.get("_id") or doc.get("bulletinFamily") or ""
    ).strip()
    title = self._ascii_safe(
        doc.get("title") or doc.get("description") or doc.get("cvelist") or ""
    ).strip()
    href = self._ascii_safe(doc.get("href") or doc.get("vhref") or "").strip()
    if not href and finding_id:
        href = "https://vulners.com/{}".format(finding_id)
    finding_type = self._ascii_safe(
        doc.get("type") or doc.get("bulletinFamily") or ""
    ).strip()
    published = self._ascii_safe(
        doc.get("published") or doc.get("publishedDate") or ""
    ).strip()

    cvss_value = 0.0
    cvss_raw = doc.get("cvss", 0.0)
    if isinstance(cvss_raw, dict):
        cvss_value = float(cvss_raw.get("score", 0.0) or 0.0)
    else:
        try:
            cvss_value = float(cvss_raw or 0.0)
        except (TypeError, ValueError):
            cvss_value = 0.0
...
def _query_vulners_for_fingerprint(self, fingerprint, api_key, max_results):
    """Query Vulners search endpoint for one fingerprint signature."""
    product = self._ascii_safe(fingerprint.get("product") or "").strip()
    version = self._ascii_safe(fingerprint.get("version") or "").strip()
    if not product:
        return [], "empty fingerprint product"

    quoted_product = '"{}"'.format(product)
    if version:
        quoted_version = '"{}"'.format(version)
        query_text = "{} AND {}".format(quoted_product, quoted_version)
    else:
        query_text = quoted_product

    params = {
        "query": query_text,
        "size": str(self._parse_positive_int(str(max_results), 20, 1, 50)),
        "apiKey": self._ascii_safe(api_key or "").strip(),
    }
    try:
        payload = self._vulners_http_get_json(
            "https://vulners.com/api/v3/search/lucene/",
            params,
            timeout_seconds=25,
        )
    except Exception as e:
        return [], self._ascii_safe(e)

    result_status = self._ascii_safe(payload.get("result") or "", lower=True).strip()
    if result_status and result_status != "ok":
        return [], self._ascii_safe(payload.get("error") or "Vulners returned error")

    data = payload.get("data", {}) or {}
    docs = data.get("search")
    if docs is None:
        docs = data.get("documents", [])
    if not isinstance(docs, list):
        return [], "unexpected Vulners response schema"

    normalized_docs = []
    for doc in docs:
        normalized = self._normalize_vulners_doc(doc)
        if normalized.get("id") or normalized.get("href") or normalized.get("title"):
            normalized_docs.append(normalized)
    return normalized_docs, ""

def _run_vulners(self, event):
    """Run Vulners enrichment against captured traffic fingerprints."""
    try:
        return heavy_runners._run_vulners(self, event)
    except Exception as e:
        err = self._ascii_safe(e)
        try:
            if hasattr(self, "vulners_area") and self.vulners_area is not None:
                self.vulners_area.append("\n[!] Vulners launcher error: {}\n".format(err))
        except Exception as ui_err:
            self._callbacks.printError(
                "Vulners launcher UI-append error: {}".format(str(ui_err))
            )
        self._callbacks.printError("Vulners launcher error: {}".format(str(e)))
        self.log_to_ui("[!] Vulners launcher error: {}".format(err))
        return None

def _export_vulners_results(self):
    """Export current Vulners findings to JSON and text files."""
    import os

    findings = list(getattr(self, "vulners_findings", []) or [])
    if not findings:
        self.vulners_area.setText(
            "[!] No Vulners findings to export\n[*] Run Vulners scan first\n"
        )
        return
    export_dir = self._get_export_dir("Vulners_Export")
    if not export_dir:
        return

    json_path = os.path.join(export_dir, "vulners_findings.json")
    txt_path = os.path.join(export_dir, "vulners_findings.txt")
    json_writer = None
    txt_writer = None
    try:
        ...
        txt_writer.write("VULNERS FINDINGS\n")
        txt_writer.write("=" * 80 + "\n")
        txt_writer.write("Total findings: {}\n\n".format(len(findings)))
        for finding in findings:
            severity = self._ascii_safe(finding.get("severity") or "info", lower=True)
            finding_id = self._ascii_safe(finding.get("id") or "").strip()
            title = self._ascii_safe(finding.get("title") or "").strip()
            signature = self._ascii_safe(finding.get("signature") or "").strip()
            href = self._ascii_safe(finding.get("href") or "").strip()
            txt_writer.write("[{}] {} {}\n".format(severity.upper(), finding_id, title))
            txt_writer.write("  Signature: {}\n".format(signature))
            txt_writer.write(
                "  CVSS: {:.1f}\n".format(float(finding.get("cvss", 0.0) or 0.0))
            )
            if href:
        ...
        self.log_to_ui("[+] Exported Vulners findings to: {}".format(export_dir))
    except Exception as e:
        self.vulners_area.append("\n[!] Vulners export failed: {}\n".format(str(e)))
        self._callbacks.printError("Vulners export failed: {}".format(str(e)))
    finally:
        if json_writer:
            try:
                json_writer.close()
                ...
                self._callbacks.printError(
                    "Error closing Vulners JSON file {}: {}".format(
                        json_path, str(close_err)
                    )
                )
        if txt_writer:
            try:
                ...
                self._callbacks.printError(
                    "Error closing Vulners text file {}: {}".format(
                        txt_path, str(close_err)
                    )
                )
...
__all__ = [
    ...
    "_extract_vulners_signature_from_value",
    "_collect_vulners_fingerprints",
    "_vulners_fetch_target_material",
    "_collect_vulners_fingerprints_from_custom_targets",
    "_vulners_http_get_json",
    "_normalize_vulners_doc",
    "_query_vulners_for_fingerprint",
    "_run_vulners",
    "_export_vulners_results",
