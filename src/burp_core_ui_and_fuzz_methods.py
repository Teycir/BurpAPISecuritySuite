...
def _export_text_output_to_ai(self, source_label, output_text):
    ...
    prompt_lines = [
        ...
        "Require non-destructive PoCs that prove exploitability without destructive writes/deletes/irreversible actions.",
        "If only destructive proof exists, mark Needs Verification and list exact missing safe validation artifacts.",
        "Use triage-proof evidence only: exact request/response deltas, auth context, UTC timing, and artifact references.",
        "Final objective: a real-world non-destructive exploit narrative with no wiggle room during severe triage.",
